...
def long_control_state_trans(CP, active, long_control_state, v_ego, v_target,
                             v_target_future, v_pid, brake_pressed, cruise_standstill, radarState):
  """Update longitudinal control state machine"""
  accelerating = v_target_future > v_target
  stopping_condition = (v_ego < 2.0 and cruise_standstill) or \
                       (v_ego < CP.vEgoStopping and
                        ((v_pid < CP.vEgoStopping and v_target_future < CP.vEgoStopping and not accelerating) or brake_pressed))
...
class LongControl():
  ...
  def update(self, active, CS, CP, long_plan, accel_limits, t_since_plan, radarState):
    ...
    self.long_control_state = long_control_state_trans(CP, active, self.long_control_state, CS.vEgo,
                                                       v_target, v_target_future, self.v_pid, CS.brakePressed,
